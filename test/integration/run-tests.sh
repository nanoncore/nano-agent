...
READ_COMMANDS="olt-status olt-alarms olt-health-check onu-list onu-info diagnose port-list port-power service-port-list discover vlan-list vlan-get profile-onu-list profile-onu-get profile-line-list profile-line-get"
WRITE_COMMANDS="onu-provision onu-delete onu-suspend onu-resume onu-reboot onu-bulk-provision port-enable port-disable vlan-create vlan-delete service-port-add service-port-delete profile-onu-create profile-onu-delete profile-line-create profile-line-delete"
...
is_unsupported_command() {
    ...
    if [[ "$vendor" == "huawei" ]]; then
        case "$cmd" in
            olt-status|olt-alarms|olt-health-check|onu-list|onu-info|port-list|port-power|service-port-list|discover|vlan-list|vlan-get|port-enable|port-disable|vlan-create|vlan-delete|service-port-add|service-port-delete|profile-onu-list|profile-onu-get|profile-onu-create|profile-onu-delete|profile-line-list|profile-line-get|profile-line-create|profile-line-delete)
                return 0
                ;;
            *)
                return 1
