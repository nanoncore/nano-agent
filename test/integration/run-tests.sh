...
WRITE_COMMANDS="onu-provision onu-delete onu-suspend onu-resume onu-reboot onu-bulk-provision port-enable port-disable"
...
requires_cli() {
    ...
    if [[ "$vendor" == "vsol" ]]; then
        case "$cmd" in
            provision|delete|reboot|configure|discover|diagnose|vlan-list|vlan-get)
