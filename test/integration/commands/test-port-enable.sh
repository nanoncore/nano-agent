...
log_info "Testing port-enable for vendor: $VENDOR (protocol: $PROTOCOL)"
...
HELP_OUTPUT=$("$BINARY" port-enable --help 2>&1) || true
...
set +e
ERROR_OUTPUT=$("$BINARY" port-enable $CMD_ARGS 2>&1)
EXIT_CODE=$?
set -e
