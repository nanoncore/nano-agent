...
log_info "Testing onu-suspend for vendor: $VENDOR (protocol: $PROTOCOL)"
...
HELP_OUTPUT=$("$BINARY" onu-suspend --help 2>&1) || true
...
set +e
ERROR_OUTPUT=$("$BINARY" onu-suspend $CMD_ARGS 2>&1)
EXIT_CODE=$?
set -e
...
# =============================================================================
# Test 3: Suspend existing ONU (integration)
# =============================================================================
log_info "Test 3: Suspend existing ONU (integration)"

if [[ "$VENDOR" != "vsol" ]]; then
    log_warn "Test 3 skipped: only validated for vsol simulator"
else
    TARGET_PON_PORT="${VSOL_SUSPEND_PON_PORT:-0/1}"
    TARGET_ONU_ID="${VSOL_SUSPEND_ONU_ID:-1}"
    TARGET_SERIAL="${VSOL_SUSPEND_ONU_SERIAL:-FHTT5929E410}"

    log_info "Suspending ONU ${TARGET_PON_PORT} ${TARGET_ONU_ID} (serial: ${TARGET_SERIAL})"

    set +e
    SUSPEND_OUTPUT=$("$BINARY" onu-suspend $CMD_ARGS \
        --pon-port "$TARGET_PON_PORT" \
        --onu-id "$TARGET_ONU_ID" 2>&1)
    SUSPEND_CODE=$?
    set -e

    if [[ $SUSPEND_CODE -ne 0 ]]; then
        log_error "Test 3 failed: suspend command returned non-zero"
        log_error "$SUSPEND_OUTPUT"
        exit 1
    fi
    ...
    if command -v snmpwalk &> /dev/null && [[ -n "$COMMUNITY" ]]; then
        if [[ "$TARGET_PON_PORT" =~ ^0/([0-9]+)$ ]]; then
            ...
            if [[ "$VERIFIED" != "true" ]]; then
                log_error "Test 3 failed: ONU rx power did not become N/A after suspend"
                log_error "$RX_POWER"
                exit 1
            fi
            log_info "Test 3 passed: ONU rx power is N/A (suspended)"
        else
...
# Summary
# =============================================================================
log_success "All onu-suspend tests passed for $VENDOR"
exit 0
